...
def render(supabase_client):
    ...
    st.markdown("""
        <div class="page-title">Prediction <span style="color:#3B82F6;">Dashboard</span></div>
        <div class="page-subtitle" style="color:#64748B;">Visual analytics of all recorded predictions</div>
    ...
    try:
        ...
        with r1c1:
            st.markdown('<p class="section-label">Predicted Sales Over Time</p>', unsafe_allow_html=True)
        ...
        with r1c2:
            st.markdown('<p class="section-label">Avg Sales by Outlet Type</p>', unsafe_allow_html=True)
            ot = log_df.groupby("Outlet_Type")["Predicted_Sales"].mean().reset_index()
            ot.columns = ["Outlet Type", "Avg Sales (₹)"]
            st.bar_chart(ot.set_index("Outlet Type"), color="#10B981")

        r2c1, r2c2 = st.columns(2)
        with r2c1:
            st.markdown('<p class="section-label">Avg Sales by Item Type</p>', unsafe_allow_html=True)
            it = log_df.groupby("Item_Type")["Predicted_Sales"].mean().reset_index()
            it.columns = ["Item Type", "Avg Sales (₹)"]
            st.bar_chart(it.set_index("Item Type"), color="#10B981")

        with r2c2:
            st.markdown('<p class="section-label">Avg Sales by Location Tier</p>', unsafe_allow_html=True)
            lt = log_df.groupby("Outlet_Location_Type")["Predicted_Sales"].mean().reset_index()
            lt.columns = ["Location Tier", "Avg Sales (₹)"]
            st.bar_chart(lt.set_index("Location Tier"), color="#F59E0B")

        r3c1, r3c2 = st.columns(2)
        with r3c1:
            if len(log_df) >= 3:
                st.markdown('<p class="section-label">Item MRP vs Predicted Sales</p>', unsafe_allow_html=True)
        ...
        with r3c2:
            st.markdown('<p class="section-label">Avg Sales by Fat Content</p>', unsafe_allow_html=True)
            fc = log_df.groupby("Item_Fat_Content")["Predicted_Sales"].mean().reset_index()
            fc.columns = ["Fat Content", "Avg Sales (₹)"]
            st.bar_chart(fc.set_index("Fat Content"), color="#F59E0B")
